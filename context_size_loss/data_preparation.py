...
class RISEDataPreparation:
    ...
    def extract_sprintf_snippets(self) -> List[Dict[str, Any]]:
        ...
        try:
            # Run git grep to find sprintf calls with context
            result = subprocess.run([
                "git", "grep", "-n", "-A", "10", "-B", "10", "sprintf"
            ], cwd=self.rise_repo_dir, capture_output=True, text=True)
        ...
        # Parse the output
        snippets = self._parse_git_grep_output(result.stdout)
        logger.info(f"Extracted {len(snippets)} sprintf code snippets")
        
        return snippets
        
    def _parse_git_grep_output(self, output: str) -> List[Dict[str, Any]]:
        """
        ...
        for line in output.split('\n'):
            ...
            # Check if this is a new file/line match
            if ':' in line and not line.startswith('-') and not line.startswith('+'):
                ...
                if len(parts) >= 2:
                    ...
                    current_snippet = {
                        "file_path": file_path,
                        "sprintf_line": line_number,
                        "context_lines": [],
                        "raw_content": line
                    }
                    
            elif current_snippet and (line.startswith('-') or line.startswith('+') or line.startswith(' ')):
